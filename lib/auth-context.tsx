...
import { getUserProfile, getBusinessProfile, signOutUser } from "@/lib/auth-client" // Corrected import for signOut
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchUserProfile = useCallback(
    async (userId: string) => {
      ...
      try {
        const data = await getUserProfile(userId) // Use function from auth-client
    ...
    [], // No dependency on supabase here, as getUserProfile handles it
  )
  ...
  const fetchBusinessProfile = useCallback(
    async (userId: string) => {
      ...
      try {
        const data = await getBusinessProfile(userId) // Use function from auth-client
    ...
    [], // No dependency on supabase here, as getBusinessProfile handles it
  )
